...
class GPT35TurboBot(LineBot):
    def __init__(self, fast_api: FastAPI):
        super().__init__(fast_api, '/linebot/gpt35turbo')
    ...
    def handle_message(self, event: MessageEvent, dest) -> SendMessage:
        ...
        if (user_msg in {"reset", "リセット"}):
            ...
            # 会話を更新（ユーザ発言）
            conversation = self.add_user_msg_to_conversation(line_user_id, user_msg)
    ...
    def check_conversation(self, line_user_id) -> List[dict]:
        if line_user_id not in conversations:
            user_name = self.get_user_name(line_user_id)
            bot_name = self.get_bot_name()
            conversations[line_user_id] = [
                {'role': 'system',
                 'content': f"あなたは「{bot_name}」という名前のLINEチャットボットです。userの名前は{user_name}です。"},
            ]
        return conversations[line_user_id]

    def get_conversation(self, line_user_id) -> List[dict]:
        self.check_conversation(line_user_id)
        return conversations[line_user_id]

    def add_ai_msg_to_conversation(self, line_user_id, ai_msg) -> List[dict]:
        conversation = self.check_conversation(line_user_id)
        conversation.append({'role': 'assistant', 'content': ai_msg})

    def add_user_msg_to_conversation(self, line_user_id, usr_msg) -> List[dict]:
        conversation = self.check_conversation(line_user_id)
        conversation.append({'role': f'user', 'content': usr_msg})
        return conversation
    ...
    def get_bot_name(self):
        try:
            profile = self.linebot_api.get_bot_info()
            return profile.display_name
        except:
            return f"会話AI"
